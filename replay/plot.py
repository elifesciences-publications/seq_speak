...
def pc_inh(rslt, smoothness=1):
    """Plot PC and INH raster and firing rates together."""
    fig, axs = plt.subplots(2, 1, figsize=(15, 6.5), tight_layout=True)
    ...
    axs[1].plot(rslt.ts, smooth(spks_pc.sum(axis=1) / (rslt.dt * rslt.p['N_PC']), smoothness), c='k', lw=3)
    axs[1].set_xlabel('Time (s)')
    axs[1].set_ylabel('PC spike rate (Hz)')
    ...
    axs[0].scatter(t_spks_inh, -(1 + nrn_spks_inh), s=5, c='r')
    axs[0].set_xlabel('Time (s)')
    axs[0].set_ylabel('Neuron')
    
    # inh population average
    ax_1_twin = axs[1].twinx()
    ax_1_twin.plot(rslt.ts, smooth(spks_inh.sum(axis=1) / (rslt.dt * rslt.p['N_INH']), smoothness), c='r', lw=2)
    ax_1_twin.set_ylabel('INH spike rate (Hz)')
    
    axs[1].set_zorder(ax_1_twin.get_zorder() + 1)
    axs[1].patch.set_visible(False)
    
    set_colors(ax_1_twin, 'r')
    
    for ax in list(axs) + [ax_1_twin]:
        ax.set_xlim(0, rslt.ts[-1])
        set_font_size(ax, 16)
...
def raster(rslt, xys, colors, cmap, nearest, epoch, trg_plt, y_lim, y_ticks, n_t_ticks=None, fig_size=(15, 3), title=None):
    """
